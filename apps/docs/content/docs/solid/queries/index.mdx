...
Settings queries (`useListAccounts`, `useAccountInfo`, `useListSessions`, `useListDeviceSessions`, `useListPasskeys`, `useListApiKeys`) are keyed per-user and wait for the active session before firing. Solid hooks stay disabled until the required user-scoped inputs resolve (`userId`, and `accountId` for `useAccountInfo`), while preserving consumer `enabled` controls. This means they drop out of the cache cleanly on sign-out and on account switching with a persister they stay isolated per user.
