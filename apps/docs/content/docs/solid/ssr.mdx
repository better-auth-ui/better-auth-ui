...
The same pattern works for Solid read helpers that expose client-shaped loader helpers — for example `prefetchListAccounts`, `prefetchListApiKeys`, and `prefetchActiveOrganization` live in `@better-auth-ui/solid` next to their options factories. When you want to call the Better Auth server instance directly instead, use the matching server-auth helpers from `@better-auth-ui/core/server`.
