...
```bash
bunx shadcn@latest add https://better-auth-ui.com/r/oauth-provider.json
```
...
Keep these paths aligned with the server's `consentPage` and `selectAccount.page`. If your route already resolves plugin paths without an allowlist, no route change is needed.
