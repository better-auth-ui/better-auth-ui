...
Use [`usePublicOAuthClient`](/docs/solid/queries/public-oauth-client) to load application metadata and [`useOAuthConsent`](/docs/solid/mutations/oauth-consent) to submit the user's decision. Both hooks are available from `@better-auth-ui/solid/plugins/oauth-provider`.
