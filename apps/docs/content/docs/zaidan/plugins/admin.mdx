...
The action renders only when `session.impersonatedBy` is present. Selecting it
calls `authClient.admin.stopImpersonating()` and refreshes the cached session
before the pending state completes.
...
Use `stopImpersonatingOptions()` from `@better-auth-ui/solid` when you need the
same behavior outside the copied user button. The options restore the admin
session and await invalidation of the shared session query.
