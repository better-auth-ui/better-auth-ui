...
`<ApiKeys />` lets users create, rename, and delete keys. The create form exposes the fields that Better Auth accepts from the client, including the name, configuration, expiration, organization, and metadata. The list shows status, remaining requests, request usage, and the last request time as read-only values.
