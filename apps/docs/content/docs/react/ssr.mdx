...
The same pattern works for every settings read — `prefetchListAccounts`, `prefetchListSessions`, `prefetchListDeviceSessions`, `prefetchListPasskeys`, and `prefetchAccountInfo` all live in `@better-auth-ui/core` next to their options factories.
...
Session server-auth helpers are canonical from `@better-auth-ui/core/server` as `sessionOptionsServer`, `ensureSessionServer`, `prefetchSessionServer`, and `fetchSessionServer`. These helpers accept your Better Auth server instance (`auth`) and request params instead of a browser `authClient`.
