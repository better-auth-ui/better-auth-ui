...
For server loaders, import from `@better-auth-ui/core/server`. Server helpers use the Better Auth server endpoint shape: pass the Better Auth server instance, signed-in `userId`, and request params with the permission payload under `body`.

```ts
import { ensureHasPermission } from "@better-auth-ui/core/server"
