...
For server loaders, import from `@better-auth-ui/core/server`. Pass the Better Auth server instance and signed-in `userId`; request headers are forwarded to Better Auth for session-aware calls.

```ts
import { ensureListUserInvitations } from "@better-auth-ui/core/server"
