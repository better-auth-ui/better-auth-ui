...
For server loaders, import from `@better-auth-ui/core/server`. The server helper calls your Better Auth server instance directly and accepts the same request params forwarded to `auth.api.getFullOrganization`.

```ts
import { ensureFullOrganization } from "@better-auth-ui/core/server"
