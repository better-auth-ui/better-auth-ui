...
For server loaders, import from `@better-auth-ui/core/server`. The server helper uses your Better Auth server instance and request params so the prefetched key hydrates into `useActiveOrganization`.

```ts
import { ensureActiveOrganization } from "@better-auth-ui/core/server"
