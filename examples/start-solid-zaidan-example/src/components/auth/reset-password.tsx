...
export function ResetPassword(props: ResetPasswordProps) {
  ...
  const form = createAuthForm(() => ({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (!token) return
      try {
        await resetPassword.mutateAsync({ token, newPassword: value.password })
      } catch {
  ...
  return (
    <Card class={cn("w-full max-w-sm", props.class)}>
      ...
      <CardContent>
        <form.AppForm>
          <form.AuthFormRoot aria-label="Reset password">
            <div class="flex flex-col gap-6">
              ...
              <form.AuthFormSubmitButton disabled={resetPassword.isPending}>
                {auth.localization.auth.resetPassword}
