...
const Slider = <T extends ValidComponent = "div">(rawProps: SliderProps<T>) => {
  const props = mergeProps(
    { minValue: 0, maxValue: 100 } as SliderProps<T>,
    rawProps
  )
...
type SliderTrackComponentProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T, SliderTrackProps<T>> &
    Pick<ComponentProps<T>, "class" | "children">

const SliderTrack = <T extends ValidComponent = "div">(
  props: SliderTrackComponentProps<T>
) => {
  const [local, others] = splitProps(props as SliderTrackComponentProps, [
    "class",
    "children"
  ])
  ...
  return (
    <SliderPrimitive.Track
      ...
      class={cn(
        "relative z-slider-track grow select-none overflow-hidden",
        local.class
      )}
      {...others}
    >
...
type SliderFillComponentProps<T extends ValidComponent = "div"> =
  PolymorphicProps<T, SliderFillProps<T>> & Pick<ComponentProps<T>, "class">

const SliderFill = <T extends ValidComponent = "div">(
  props: SliderFillComponentProps<T>
) => {
  const [local, others] = splitProps(props as SliderFillComponentProps, [
    "class"
  ])
  const context = useSliderContext()
  return (
...
type SliderThumbComponentProps<T extends ValidComponent = "span"> =
  PolymorphicProps<T, SliderThumbProps<T>> & Pick<ComponentProps<T>, "class">

const SliderThumb = <T extends ValidComponent = "span">(
  props: SliderThumbComponentProps<T>
) => {
  const [local, others] = splitProps(props as SliderThumbComponentProps, [
    "class"
  ])
  return (
