...
const meta = {
  id: "heroui-plugins-organization",
  title: "HeroUI/Plugins/Organization",
...
export const OrganizationSwitcherPreview: Story = {
  ...
  play: async ({ canvas, step, userEvent }) => {
    await step("open the organization switcher", async () => {
      const trigger = canvas.getByRole("button", { name: "Organization" })
...
export const EditTeamDialogPreview: Story = {
  ...
  play: async ({ canvas, canvasElement, step, userEvent }) => {
    await step("open team management", async () => {
      ...
      await waitFor(() => {
        expect(body.getByDisplayValue("Platform")).toBeVisible()
        expect(body.getByText("Ada Lovelace")).toBeVisible()
        expect(body.getByText("Grace Hopper")).toBeVisible()
      })
    })
  }
}
...
export const EditRoleDialogPreview: Story = {
  ...
  play: async ({ canvas, canvasElement, step, userEvent }) => {
    await step("open role editing", async () => {
      ...
      const body = within(canvasElement.ownerDocument.body)
      await waitFor(() => {
        expect(body.getByDisplayValue("billing")).toBeVisible()
        expect(body.getByText("Invoices")).toBeVisible()
      })
    })
  }
}
