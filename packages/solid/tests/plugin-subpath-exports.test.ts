...
describe("Solid plugin subpath exports", () => {
  it("publishes plugin-specific hooks, mutations, and queries from plugin-scoped entrypoints", () => {
    ...
    expect(sso).toHaveProperty("useRegisterSsoProvider")
    expect(sso).toHaveProperty("useRequestSsoDomainVerification")
    expect(sso).toHaveProperty("useSignInSso")
    expect(sso).toHaveProperty("useVerifySsoDomain")
  ...
  it("keeps core-owned mutation factories out of framework plugin entrypoints", () => {
    ...
    expect(sso).not.toHaveProperty("registerSsoProviderOptions")
    expect(sso).not.toHaveProperty("requestSsoDomainVerificationOptions")
    expect(sso).not.toHaveProperty("signInSsoOptions")
    expect(sso).not.toHaveProperty("verifySsoDomainOptions")
  ...
  it("does not publish plugin-specific APIs from the root entrypoint", async () => {
    ...
    expect(solid).not.toHaveProperty("useDashAuditLogs")
    ...
    expect(solid).not.toHaveProperty("useStopImpersonating")
    expect(solid).not.toHaveProperty("useVerifyTotp")
    expect(solid).not.toHaveProperty("useVerifyPhoneNumber")
  })
})
